...
func shouldPreserveOpenAIResponsesNoneReasoningEffort(account *Account) bool {
	return account != nil && (account.IsOpenAIOAuthLike() || shouldPreserveOpenAIResponsesOptionalFields(account))
}
...
// action=force_priority writes "priority" even when the field is absent, and
// rewrites any matched known tier to "priority". Before the global policy is
// evaluated, a trusted request Group with ForceOpenAIFast enabled
// unconditionally sets service_tier to "priority". The global policy remains
// authoritative and may still pass, filter, or block that final value.
//
