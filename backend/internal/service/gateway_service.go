...
// resolveCompositeModelOwnership derives a provider from the public model
// mappings configured on schedulable accounts in a composite group. Empty
// mappings are intentionally ignored here: they are a compatibility fallback
// for single-provider groups, not an explicit claim over every provider model.
// Wildcard keys (`*` / `gpt-*`) are also ignored: they describe forwarding
// fallbacks, not ownership of every matching public ID.
