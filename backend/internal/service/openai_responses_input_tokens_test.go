...
func TestForwardResponsesInputTokensGrokAndCNUseLocalEstimate(t *testing.T) {
	gin.SetMode(gin.TestMode)

	for _, account := range []*Account{
		{ID: 160, Platform: PlatformGrok, Type: AccountTypeOAuth},
		{ID: 161, Platform: PlatformDeepSeek, Type: AccountTypeAPIKey, Credentials: map[string]any{"api_key": "ds-key"}},
		{ID: 162, Platform: PlatformKimi, Type: AccountTypeAPIKey, Credentials: map[string]any{"api_key": "kimi-key"}},
	} {
		t.Run(account.Platform, func(t *testing.T) {
			...
			body := []byte(`{"model":"codex-model","input":"hello world"}`)
