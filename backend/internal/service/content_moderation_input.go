...
func (collector moderationTextCollector) collectLastResponsesInput(input gjson.Result, parts *[]string, images *[]string) {
	...
	case input.IsArray():
		...
		if !collector.filterReminders {
			last := array[len(array)-1]
			if last.Get("type").String() == "compaction_trigger" && len(array) > 1 {
		...
		if compactionRequest {
			for lastIndex--; lastIndex >= 0; lastIndex-- {
				if collector.isResponsesUserTextItem(array[lastIndex]) {
					break
				}
			}
		}
		...
		for _, item := range array[:toolOutputEnd] {
			if isResponsesToolOutputType(item.Get("type").String()) {
				collector.collectResponsesToolOutput(item.Get("output"), parts, images)
			}
		}
	case input.IsObject():
		...
		if collector.filterReminders && isResponsesToolOutputType(input.Get("type").String()) {
			collector.collectResponsesToolOutput(input.Get("output"), parts, images)
		}
	}
}
...
func (collector moderationTextCollector) collectResponsesToolOutput(output gjson.Result, parts *[]string, images *[]string) {
	partsBefore := len(*parts)
	imagesBefore := len(*images)
	seen := make(map[string]struct{})
	collector.collectResponsesToolOutputValue(output, parts, images, seen)
	if len(*parts) != partsBefore || len(*images) != imagesBefore || !output.Exists() {
		return
	}
	if raw := strings.TrimSpace(output.Raw); raw != "" && raw != "null" {
		collector.addModerationText(parts, raw)
	}
}
...
func (collector moderationTextCollector) collectResponsesToolOutputValue(value gjson.Result, parts *[]string, images *[]string, seen map[string]struct{}) {
	switch {
	case !value.Exists():
		return
	case value.Type == gjson.String:
		...
		partsBefore := len(*parts)
		collector.addModerationText(parts, text)
		if len(*parts) != partsBefore {
			seen[text] = struct{}{}
		}
	case value.IsArray():
		value.ForEach(func(_, item gjson.Result) bool {
			collector.collectResponsesToolOutputValue(item, parts, images, seen)
			return true
		})
	case value.IsObject():
		...
		value.ForEach(func(key, child gjson.Result) bool {
			...
			collector.collectResponsesToolOutputValue(child, parts, images, seen)
			return true
		})
	}
}
