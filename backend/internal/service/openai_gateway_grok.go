...
func GrokSupportsXHighReasoningEffort(model string) bool {
	canonical := xai.ResolveGrokTextResponsesModelID(strings.ToLower(xai.StripGrokProviderPrefix(strings.TrimSpace(model))))
	return canonical == grokExtraHighReasoningModel || canonical == "grok-4.7"
}
