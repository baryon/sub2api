...
func (s *BillingService) calculateTokenCost(resolved *ResolvedPricing, input CostInput) (*CostBreakdown, error) {
	...
	// 长上下文计费必须同时通过分组策略和账号策略；账号未显式声明时仅使用分组配置。
...
func (s *BillingService) applyModelSpecificPricingPolicyEx(model string, pricing *ModelPricing, forceDeepSeekRates bool, pricingAt time.Time) *ModelPricing {
	...
	_, hasMaxReasoningMultiplier := pricing.ReasoningEffortMultipliers["max"]
	needsFable51MaxMultiplier := isClaudeFable51Model(model) && !hasMaxReasoningMultiplier
	fastRatio := openAIModelFastPricingRatio(model)
	needsOpus55FastMultiplier := claude.IsOpus55(model) && pricing.FastMultiplier == nil
	if !needsCacheCreationPolicy && fastRatio <= 0 && !needsOpus55FastMultiplier && !needsFable51MaxMultiplier {
		return pricing
	}
	cloned := *pricing
	if needsFable51MaxMultiplier {
		cloned.ReasoningEffortMultipliers = maps.Clone(pricing.ReasoningEffortMultipliers)
		if cloned.ReasoningEffortMultipliers == nil {
			cloned.ReasoningEffortMultipliers = make(map[string]float64, 1)
		}
		cloned.ReasoningEffortMultipliers["max"] = 3
	}
