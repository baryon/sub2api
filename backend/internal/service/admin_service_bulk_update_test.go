...
func TestAdminServiceBulkUpdateAccounts_NormalizesDeepSeekCredentialsByIDs(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, 1, result.Success)
	require.Equal(t, "rotated-key", repo.lastBulkUpdate.Credentials["api_key"])
	require.NotContains(t, repo.lastBulkUpdate.Credentials, "base_url", "an omitted base_url must preserve each target account's existing value")
}
